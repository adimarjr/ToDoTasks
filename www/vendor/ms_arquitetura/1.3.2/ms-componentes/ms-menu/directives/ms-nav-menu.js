define([
		...
		msMenu.directive('msNavMenu', ['$compile', 'msRouteService', '$http', '$parse',
                    function($compile, msRouteService, $http, $parse) {
                                    ...
                                    msRouteService.create(val).then(function(result) {
                                        ...
                                        var template = angular.element(
                                                        '<ul class="nav">' +
                                                                ...
                                                                '<a ng-if="!node.children" ng-click="reloadState((node.state && node.state.url) ? node.state.url : (node.text | msRemoverAcentuacao | msHifenizar))" ng-bind-html="node.text"></a>' + 
